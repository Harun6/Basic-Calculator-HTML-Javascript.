    ...
    if (TxtResult !== undefined) {
        if (TxtResult !== null) { 
            SecondNumber = parseFloat(TxtResult.value.replace(',', '.')); // By adding the replace method, parts with commas were made functional.
             // The parseFloat function is used to convert the value it receives to a number type. Returns the first number if there is a space between the numbers. If it cannot convert the value it receives to a number type, it returns NaN.
        }
    }
    if (Process === '+') {
